...
class VSM:
    def __init__(self, model_dir = 'model'):
        ...
        # Select only number part
        with open(self.filelist_path, 'r', encoding='utf-8') as file:
            self.filelist = np.array([ int(re.search('D([0-9]+)', _v.strip()).group(1)) for _v in file.readlines()])
    ...
    def retrieval(self, query):
        ...
        return scores_follow_filelist, descending_ranking_by_id, self.filelist
