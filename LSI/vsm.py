...
class VSM:
    def __init__(self, model_dir = 'model', dim=100):
        ...
        with open(self.vocab_path, 'r', encoding='utf-8') as file:
            ...
            self.vocab = [_v.strip() for _v in file.readlines()]
        ...
        self.doc_num = self.filelist.shape[0]
    ...
    def SVD(self, dim=100):
        ...
        for term in self.TF:
            for doc in self.TF[term]:
                row.append(self.vocab2id[term])
                col.append(int(doc))
                data.append(self.TF[term][doc]*self.IDF[term])
        tfidf_matrix = csc_matrix((data, (row, col)), shape=(self.term_num+1, self.doc_num))
        start_time = time.time()
        u, s, vt = svds(tfidf_matrix, k=dim)
        print(round(time.time() - start_time, 2), 'sec')
        eigen_m = np.zeros((dim, dim))
        np.fill_diagonal(eigen_m, s)
        self.approx = u@eigen_m@vt
        print(self.approx.shape)
    def retrieval(self, query):
        ...
        #     Calculate term frequancy in query, qtf
            ...
            self.scores = self.get_document_score_in_approx()
            
            scores_index = np.argsort(self.scores, axis=0)
            scores_index = np.argsort(self.scores, axis=0)[::-1]
            scores_follow_filelist.append(np.sort(self.scores)[::-1])
    ...
    def get_document_score_in_approx(self):
        scores = np.zeros(self.filelist.shape)
        for term in self.QTF:
            term_id = self.vocab2id[term]
            for file_id in range(self.doc_num):
                scores[file_id] += self.approx[term_id][file_id]* (((k3 + 1) * self.QTF[term]) / (k3 + self.QTF[term]))
        return scores